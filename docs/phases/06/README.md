...
## The discussion around the lab

The lab remains the proven tournament described below. The lecture uses its results to explore several larger ideas; these are discussion frameworks, not additional build requirements:

- why generous personal subscriptions should not become a permanent production-cost assumption,
- why the harness around a model can matter as much as the model,
- where rapidly improving local and quantized models are already useful,
- how organizations can name stable roles such as utility, workhorse, specialist, and deep thinker,
- how reasoning effort can be discussed separately from model capability,
- and how model routing can eventually choose an appropriate level of capability, speed, thought, cost, privacy, and verification.

OpenRouter may be mentioned as one early market example because the instructor has direct experience with it and reported acquisition interest illustrates the growing value of routing. It is not used, recommended, or implemented by this phase.
...
The broader conclusion is not that one model wins forever. The evaluation evidence can help a team describe which current models fit which levels of work, while leaving future routing implementations and vendors open.
