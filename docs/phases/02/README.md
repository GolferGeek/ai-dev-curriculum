...
## Scheduled hygiene — the closing bracket on a cron

Interactive gates (`/scan-errors` → `/commit pr`) only run when someone invokes them. **Nightly hygiene** is the repo immune system: GitHub Actions on a schedule, same proof chain, **PR for human merge**.

| Tier | What runs | When to enable |
|------|-----------|----------------|
| **1** | `build · lint · test` on cron | First — no agent API key |
| **2** | Scan → fix → monitor → harden → maintenance branch + PR | After Tier 1 is green and local `/nightly-hygiene` succeeds |

**Client sheet:** [Guardrails G1 — Nightly hygiene](../../../marketing/adoption-kit/guardrails/01-nightly-hygiene-github-actions.md) · **Full handbook:** [docs/github/actions-and-agents.md](../../github/actions-and-agents.md) (working workflows in `.github/workflows/`). **Skill:** `/nightly-hygiene`.

SaaS product code and hygiene workflows can have different owners. Auto-merge to `main` is out of scope unless kit 05 documents an explicit exception.
