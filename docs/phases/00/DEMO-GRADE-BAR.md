...
- **Tests** that follow a user-visible path through the loop, not only `document.title`. On the **first effort** for a slice, expect **unit**, **API/HTTP** (curl-style route checks where the track has an API), **end-to-end** (Playwright), then **browser** verification — not a single landing-page assertion.
