...
This is the capstone and the most future-consequential phase in the curriculum. Everything you've learned — building apps, maintaining quality, researching codebases, creating your own tools — comes together in one demo that shows the **future of how software and business work**: autonomous agents representing people and companies, discovering each other, delegating tasks, streaming results, and transacting under policy.

And you get to watch it all happen in real time.

Start with [OVERVIEW.md](./OVERVIEW.md), then read [AGENT-TO-AGENT-FUTURE.md](./AGENT-TO-AGENT-FUTURE.md). In the room, use [TEACHING.md](./TEACHING.md) for the learning story and [PRESENTER-GUIDE.md](./PRESENTER-GUIDE.md) for the slide-by-slide delivery. The restaurant demo is deliberately small; the real subject is every company becoming safely reachable by customer, candidate, vendor, and partner agents.
...
You already know this one — it's how an AI application connects to tools, data, prompts, and resources. MCP is not literally one-directional; it supports notifications and richer client/server interaction. The useful distinction is **capability versus counterparty**: MCP equips an agent with tools, while A2A lets independent agents delegate goals and manage tasks together.
...
- **Phase 04** shows you **where business is going** — people and companies represented by agents that discover, collaborate, negotiate, and transact using open protocols

The first rung is simple: make a website return clean Markdown or structured content to an agent. The larger shift is persistent business relationships—candidate agent to company agent, buyer agent to supplier agent, customer agent to service agent—with identity, authority, reputation, rate limits, audit, and human intervention.

Protocol partner counts and product claims change quickly; refresh them from the primary sources before teaching. The durable lesson is not a count. It is that every company needs an agent interface and a policy for what that interface may do.

## Phase documents

- [OVERVIEW.md](./OVERVIEW.md) — learner-facing session anchor
- [AGENT-TO-AGENT-FUTURE.md](./AGENT-TO-AGENT-FUTURE.md) — strategic centerpiece
- [PREREQUISITES.md](./PREREQUISITES.md) — setup and safety
- [COMMANDS.md](./COMMANDS.md) — build, run, and inspection reference
- [STARTER-KIT.md](./STARTER-KIT.md) — services, roles, and company-design worksheet
...
- [Lesson plan](../../../marketing/lesson-plans/phase-04.md) — lecture and sources
