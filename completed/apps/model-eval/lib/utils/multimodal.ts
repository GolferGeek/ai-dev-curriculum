...
function readImageAsBase64(imagePath: string): string {
  const imageRoot = resolve(process.cwd(), "test-images");
  const relativePath = imagePath.replace(/^test-images[\\/]/, "");
  const absolutePath = resolve(imageRoot, relativePath);
  if (
    absolutePath !== imageRoot &&
    !absolutePath.startsWith(`${imageRoot}${sep}`)
  ) {
    throw new Error("Image path must remain inside test-images");
  }
